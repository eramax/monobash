...
pub const meta = core.AppletMeta{ .name = "add-shell", .main = main };

const shells_file = "/etc/shells";

pub fn main(args: [][]const u8) u8 {
    if (args.len < 2) return core.die(1, "usage: add-shell SHELL...\n", .{});
    ...
    if (out_fd < 0) return core.die(1, "add-shell: cannot create temp file\n", .{});
    ...
    const out_fd2 = core.c.open(tmp_fn_z.ptr, core.c.O_WRONLY);
    _ = core.c.close(out_fd2);

    if (core.c.rename(tmp_fn_z.ptr, orig_fn_z.ptr) < 0) {
        _ = core.c.unlink(tmp_fn_z.ptr);
        return core.die(1, "add-shell: rename failed\n", .{});
    }
