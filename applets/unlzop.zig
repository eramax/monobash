...
pub const meta = core.AppletMeta{ .name = "unlzop", .main = main };

fn decompressToFd(data: []const u8, out_fd: c_int) u8 {
    const alloc = std.heap.page_allocator;
    var child = std.process.Child.init(&[_][]const u8{ "lzop", "-d" }, alloc);
    child.stdin_behavior = .Pipe;
    child.stdout_behavior = .Pipe;
    child.stderr_behavior = .Inherit;
    child.spawn() catch return core.die(1, "unlzop: cannot spawn lzop\n", .{});
    child.stdin.?.writeAll(data) catch {
        _ = child.wait();
        return core.die(1, "unlzop: write error\n", .{});
    };
    child.stdin.?.close();
    const output = child.stdout.?.reader().readAllAlloc(alloc, 1 << 28) catch {
        _ = child.wait();
        return core.die(1, "unlzop: read error\n", .{});
    };
...
pub fn main(args: [][]const u8) u8 {
    ...
    while (i < args.len and std.mem.eql(u8, args[i], "--")) {
        ...
        return core.die(1, "unlzop: not implemented (no lzop in Zig std lib)\n", .{});
    }
    const alloc = std.heap.page_allocator;
    if (i >= args.len) {
    ...
    while (i < args.len) {
        ...
        const fd = core.openReadName(file) orelse {
            rc |= core.die(1, "unlzop: cannot open '{s}'\n", .{file});
            continue;
        };
        ...
        defer alloc.free(data);
        var out_name = file;
        if (std.mem.endsWith(u8, file, ".lzo")) {
