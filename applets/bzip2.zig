...
pub const meta = core.AppletMeta{ .name = "bzip2", .main = main };

fn compressToFd(data: []const u8, out_fd: c_int) u8 {
    const alloc = std.heap.page_allocator;
    var child = std.process.Child.init(&[_][]const u8{ "bzip2", "-z" }, alloc);
    child.stdin_behavior = .Pipe;
    child.stdout_behavior = .Pipe;
    child.stderr_behavior = .Inherit;
    child.spawn() catch return core.die(1, "bzip2: cannot spawn bzip2\n", .{});
    child.stdin.?.writeAll(data) catch {
        _ = child.wait();
        return core.die(1, "bzip2: write error\n", .{});
    };
    child.stdin.?.close();
    const output = child.stdout.?.reader().readAllAlloc(alloc, 1 << 28) catch {
        _ = child.wait();
        return core.die(1, "bzip2: read error\n", .{});
    };
...
pub fn main(args: [][]const u8) u8 {
    ...
    while (i < args.len and args[i].len > 0 and args[i][0] == '-') {
        ...
        for (args[i][1..]) |c| {
            switch (c) {
                'd' => decompress = true,
                else => return core.die(1, "bzip2: unknown option '{c}'\n", .{c}),
            }
        }
        i += 1;
    }
    ...
    if (decompress) {
        if (file_arg) |file| {
            const fd = core.openReadName(file) orelse return core.die(1, "bzip2: cannot open '{s}'\n", .{file});
            defer _ = core.c.close(fd);
            const data = core.readAll(alloc, fd, 1 << 28) catch return 1;
            defer alloc.free(data);
            var out_name = file;
            if (std.mem.endsWith(u8, file, ".bz2")) {
                out_name = file[0 .. file.len - 4];
            }
...
fn decompressToFd(data: []const u8, out_fd: c_int) u8 {
    const alloc = std.heap.page_allocator;
    var child = std.process.Child.init(&[_][]const u8{ "bzip2", "-d" }, alloc);
    child.stdin_behavior = .Pipe;
    child.stdout_behavior = .Pipe;
    child.stderr_behavior = .Inherit;
    child.spawn() catch return core.die(1, "bzip2: cannot spawn bzip2\n", .{});
