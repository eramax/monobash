...
pub const meta = core.AppletMeta{ .name = "xz", .main = main };

fn compressToFd(data: []const u8, out_fd: c_int) u8 {
    const alloc = std.heap.page_allocator;
    var child = std.process.Child.init(&[_][]const u8{ "xz", "-z" }, alloc);
    child.stdin_behavior = .Pipe;
    child.stdout_behavior = .Pipe;
    child.stderr_behavior = .Inherit;
    child.spawn() catch return core.die(1, "xz: cannot spawn xz\n", .{});
    child.stdin.?.writeAll(data) catch {
        _ = child.wait();
        return core.die(1, "xz: write error\n", .{});
    };
    child.stdin.?.close();
    const output = child.stdout.?.reader().readAllAlloc(alloc, 1 << 28) catch {
        _ = child.wait();
        return core.die(1, "xz: read error\n", .{});
    };
...
fn decompressToFd(data: []const u8, out_fd: c_int) u8 {
    const alloc = std.heap.page_allocator;
    var child = std.process.Child.init(&[_][]const u8{ "xz", "-d" }, alloc);
    child.stdin_behavior = .Pipe;
    child.stdout_behavior = .Pipe;
    child.stderr_behavior = .Inherit;
    child.spawn() catch return core.die(1, "xz: cannot spawn xz\n", .{});
...
pub fn main(args: [][]const u8) u8 {
    ...
    while (i < args.len and args[i].len > 0 and args[i][0] == '-') {
        ...
        for (args[i][1..]) |c| {
            switch (c) {
                'd' => decompress = true,
                else => return core.die(1, "xz: unknown option '{c}'\n", .{c}),
            }
        }
        i += 1;
    }
    ...
    if (decompress) {
        if (file_arg) |file| {
            const fd = core.openReadName(file) orelse return core.die(1, "xz: cannot open '{s}'\n", .{file});
        ...
        if (file_arg) |file| {
            ...
            defer alloc.free(data);
            var buf: [4096:0]u8 = undefined;
            if (file.len + 3 >= buf.len) return 1;
            @memcpy(buf[0..file.len], file);
            buf[file.len..][0..3].* = ".xz".*;
            buf[file.len + 3] = 0;
            const out_fd = core.c.open(buf[0..file.len + 3 :0].ptr, core.c.O_WRONLY | core.c.O_CREAT | core.c.O_TRUNC, @as(c_uint, 0o644));
            if (out_fd < 0) return 1;
            defer _ = core.c.close(out_fd);
