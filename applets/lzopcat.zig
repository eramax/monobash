...
pub const meta = core.AppletMeta{ .name = "lzopcat", .main = main };

fn decompressToFd(data: []const u8, out_fd: c_int) u8 {
    const alloc = std.heap.page_allocator;
    var child = std.process.Child.init(&[_][]const u8{ "lzop", "-d", "-c" }, alloc);
    child.stdin_behavior = .Pipe;
    child.stdout_behavior = .Pipe;
    child.stderr_behavior = .Inherit;
    child.spawn() catch return core.die(1, "lzopcat: cannot spawn lzop\n", .{});
    child.stdin.?.writeAll(data) catch {
        _ = child.wait();
        return core.die(1, "lzopcat: write error\n", .{});
    };
    child.stdin.?.close();
    const output = child.stdout.?.reader().readAllAlloc(alloc, 1 << 28) catch {
        _ = child.wait();
        return core.die(1, "lzopcat: read error\n", .{});
    };
...
fn decompressFile(name: []const u8) u8 {
    ...
    const fd = core.openReadName(name) orelse {
        return core.die(1, "lzopcat: cannot open '{s}'\n", .{name});
    };
...
pub fn main(args: [][]const u8) u8 {
    ...
    while (i < args.len and std.mem.eql(u8, args[i], "--")) {
        ...
        return core.die(1, "lzopcat: not implemented (no lzop in Zig std lib)\n", .{});
    }
    if (i >= args.len) {
        const alloc = std.heap.page_allocator;
