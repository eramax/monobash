...
pub const meta = core.AppletMeta{ .name = "mkpasswd", .main = main };
...
pub fn main(args: [][]const u8) u8 {
    ...
    while (i < args.len and args[i].len > 0 and args[i][0] == '-') {
        ...
        if (args[i][1] == 'm' or args[i][1] == 'a') {
            i += 1;
            if (i >= args.len) return core.die(1, "mkpasswd: -m requires argument\n", .{});
            opt_method = args[i];
        } else if (args[i][1] == 'S') {
            i += 1;
            if (i >= args.len) return core.die(1, "mkpasswd: -S requires argument\n", .{});
            opt_salt = args[i];
        } else if (args[i][1] == 'P') {
            i += 1;
            if (i >= args.len) return core.die(1, "mkpasswd: -P requires argument\n", .{});
            opt_fd = std.fmt.parseInt(usize, args[i], 10) catch return core.die(1, "mkpasswd: invalid fd\n", .{});
        } else if (args[i][1] == 's') {
            opt_fd = 0;
        } else return core.die(1, "mkpasswd: unknown option -{c}\n", .{args[i][1]});
        i += 1;
    }
    ...
    if (password == null) {
        const is_tty = core.c.isatty(0) != 0;
        if (opt_fd) |fd| {
            const buf = core.readAll(alloc, @as(c_int, @intCast(fd)), 4096) catch return 1;
            password = std.mem.trim(u8, buf, "\n\r ");
        } else if (is_tty) {
            const p = core.c.getpass("Password: ");
            if (p) |pp| password = std.mem.sliceTo(@as([*c]u8, @ptrCast(pp)), 0);
        } else {
    ...
    if (result == null) return core.die(1, "mkpasswd: crypt failed\n", .{});
