...
pub const meta = core.AppletMeta{ .name = "lzcat", .main = main };
...
fn decompressToFd(data: []const u8, out_fd: c_int) u8 {
    ...
    var decomp = std.compress.lzma.Decompress.initOptions(
        ...
        return core.die(1, "lzcat: decompression failed\n", .{});
    };
    defer decomp.deinit();
...
fn decompressFile(name: []const u8) u8 {
    ...
    const fd = core.openReadName(name) orelse {
        return core.die(1, "lzcat: cannot open '{s}'\n", .{name});
    };
...
pub fn main(args: [][]const u8) u8 {
    ...
    while (i < args.len and std.mem.eql(u8, args[i], "--")) {
        ...
        return core.die(1, "lzcat: unknown option\n", .{});
    }
    if (i >= args.len) {
        const alloc = std.heap.page_allocator;
