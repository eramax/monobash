...
pub const meta = core.AppletMeta{ .name = "base64", .main = main };
...
fn encode(data: []const u8, alloc: std.mem.Allocator) ![]u8 {
    ...
    while (i < data.len) {
        ...
        const base = (i / 3) * 4;
        out[base + 0] = ALPHABET[(buf[0] >> 2) & 0x3F];
        out[base + 1] = ALPHABET[((buf[0] << 4) | (buf[1] >> 4)) & 0x3F];
        out[base + 2] = ALPHABET[((buf[1] << 2) | (buf[2] >> 6)) & 0x3F];
        out[base + 3] = ALPHABET[buf[2] & 0x3F];
        if (chunk < 3) {
            const pad = 3 - chunk;
            var j: usize = 0;
            while (j < pad) : (j += 1)
...
pub fn main(args: [][]const u8) u8 {
    ...
    while (i < args.len) {
        if (std.mem.eql(u8, args[i], "-d")) {
            ...
            return core.die(1, "base64: unknown option: {s}\n", .{args[i]});
        } else {
            file = args[i];
        }
        i += 1;
    ...
    const data = if (file) |f| blk: {
        ...
        if (f.len >= fbuf.len) return core.die(1, "base64: path too long\n", .{});
        @memcpy(fbuf[0..f.len], f);
        fbuf[f.len] = 0;
        const fd = core.c.open(&fbuf, core.c.O_RDONLY);
        if (fd < 0) return core.die(1, "base64: {s}: No such file\n", .{f});
