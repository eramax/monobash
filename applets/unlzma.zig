...
pub const meta = core.AppletMeta{ .name = "unlzma", .main = main };
...
fn decompressToFd(data: []const u8, out_fd: c_int) u8 {
    ...
    var decomp = std.compress.lzma.Decompress.initOptions(
        &source, alloc, buf, .{}, std.math.maxInt(usize),
    ) catch {
        return core.die(1, "unlzma: decompression failed\n", .{});
    };
    defer decomp.deinit();
...
pub fn main(args: [][]const u8) u8 {
    ...
    while (i < args.len and std.mem.eql(u8, args[i], "--")) {
        ...
        return core.die(1, "unlzma: unknown option\n", .{});
    }
    const alloc = std.heap.page_allocator;
    if (i >= args.len) {
    ...
    while (i < args.len) {
        ...
        const fd = core.openReadName(file) orelse {
            rc |= core.die(1, "unlzma: cannot open '{s}'\n", .{file});
            continue;
        };
